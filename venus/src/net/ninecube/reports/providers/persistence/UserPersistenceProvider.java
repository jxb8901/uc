...
public class UserPersistenceProvider 
{
		...
		try
		{
			...
			{			
				List list = session.createQuery(
						"from net.ninecube.reports.objects.ReportUser as user "
	...
	{
		String fromClause =
			"from net.ninecube.reports.objects.ReportUser reportUser order by reportUser.name ";
	...
	public void deleteUser(ReportUser user) throws ProviderException
	{
		...
		try
		{
			...
			session
					.createQuery(
							"DELETE net.ninecube.reports.objects.ReportLog reportLog where reportLog.user.id = ? ")
