...
import net.ninecube.reports.objects.ReportExportOption;
import net.ninecube.reports.objects.ReportParameter;
import net.ninecube.reports.objects.ReportParameterMap;
...
public class JasperReportEngine extends ReportEngine
{
	...
	private JasperReport loadJasperReport(Report report) throws ProviderException {
		String filename = directoryProvider.getReportDirectory() + report.getFile();
		try {
			return (JasperReport) JRLoader.loadObject(filename);
		}
		catch(JRException e) {
			log.error("failed to load report, try to compile it: '" + filename + "'", e);
			if (filename.endsWith("jasper")) {
				filename = filename.replaceAll("\\.jasper", ".jrxml");
			}
			try {
				return JasperCompileManager.compileReport(filename);
			} catch (JRException e1) {
				throw new ProviderException(e1);
			}
		}
	}
}
